		...
		function jump(a) {
			...
			$.ajax({
				...
				success: function(data) {
					...
					tableStr = tableStr + "<thead style='font-size:13px' id='archive-info-head'><tr class='danger'><th>文件ID</th><th>上传时间</th><th>是否需要转码</th><th>是否已转码</th><th>转码格式</th><th>是否已归档</th><th>是否已删除</th><th>归档文件夹</th></tr></thead>";
	...
	<script>
		...
		$(function() {
			...
			function showResponse(data) {
				...
				var onePageNum = 25;
				var tableStr = "<table class='table table-striped' id='archive-info' name='archive-info'><caption>未归档数据</caption>";
				tableStr = tableStr + "<thead id='archive-info-head'><tr style='font-size:13px' class='danger'><th>文件ID</th><th>上传时间</th><th>是否需要转码</th><th>是否已转码</th><th>转码格式</th><th>是否已归档</th><th>是否已删除</th><th>归档文件夹</th></tr></thead>";
				tableStr = tableStr + "<tbody id='info-body'>";
				...
				if(data.root.length > onePageNum) {
					...
					page = page + "<li id='-1'><a href='javascript:jump(" + "-1" + ")'>&laquo;</a></li>";
					var pageNum = data.root.length / onePageNum;
					if(data.root.length % onePageNum > 0) {
						pageNum = pageNum + 1;
					}
					for(var j = 1; j <= pageNum; j++) {
						page = page + "<li id=" + j + " ><a href='javascript:jump(" + j + ");'>";
						page = page + j + "</a></li>";
					}
					page = page + "<li id='0'><a href='javascript:jump(" + "0" + ")'>&raquo;</a></li></ul>";
				...
				$.ajax({
					...
					success: function(data) {
						if(data.failed.length > 0) {
							alert("archive failed id: " + data.failed);
						} else {
							alert("archive succeed");
						}
						showResponse(data);
					}
				});
				//alert(selectedData.html());
			});
...
<body>
	...
	<div class="panel panel-danger panel1" style="width: 1400px;">
		<div class="panel-heading">
			<h3 class="panel-title">归档查询</h3>
		</div>

		<div class="panel-body">
			<form class="form-inline" role="form" style="text-align:center;width:1300px;" id="archive-query">
			...
			<table class="table table-striped" style="width: 1350px" id="archive-info">
				<caption>未归档数据</caption>
				<thead>
					<tr class='danger' style='font-size:13px'>
